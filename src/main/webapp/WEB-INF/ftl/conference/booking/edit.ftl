...
<div class="container" style="padding-bottom: 15px;min-height: 300px; margin-top: 40px;">
    ...
    <div class="row">
        ...
        <div class="col-md-10">
            <h2>编辑预订</h2>
            <hr>
            <form class="col-md-8" method="post" action="${basePath}/booking/update.shtml" id="editForm">
                ...
                <div class="form-group">
                    <label>开始时间</label>
                    <input type="datetime-local" class="form-control" name="startTime" id="startTime" value="${booking.startTime}" required>
                </div>
                <div class="form-group">
                    <label>结束时间</label>
                    <input type="datetime-local" class="form-control" name="endTime" id="endTime" value="${booking.endTime}" required>
                </div>
                <div class="form-group">
                    <label>会议目的</label>
                    <textarea class="form-control" name="purpose" placeholder="请输入会议目的" required>${booking.purpose}</textarea>
                </div>
                <div class="form-group">
...
<script>
        ...
        $.post("${basePath}/conferenceRoom/dataList.shtml", {}, function(result) {
            if (result.status === 200) {
                ...
                $.each(result.list, function(index, room) {
                    if (room.status === 1) {
                        var selected = (room.id == ${booking.conferenceRoomId}) ? 'selected' : '';
                        html += "<option value='" + room.id + "' " + selected + ">" + room.name + "（" + room.capacity + "人）</option>";
                    }
                });
    ...
    $("#editForm").submit(function(e) {
        ...
        $.post("${basePath}/booking/update.shtml", formData, function(result) {
            if (result.status === 200) {
                layer.msg(result.message);
                setTimeout(function() {
